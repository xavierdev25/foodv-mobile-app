...
export async function request(endpoint, method = "GET", body = null, auth = false, token = null) {
    ...
    console.log("FETCH:", `${BASE_URL}${endpoint}`, { method, headers, body });
...
export async function createOrder(data, token) {
    return request("/orders", "POST", data, true, token);
}

export async function getUserOrders(token) {
    return request("/orders", "GET", null, true, token);
}

//USERS
//mmc: Como es front para el usuario, solo sería editar tus datos. Pero el servicio web tendría acceso a los otros endpoints
